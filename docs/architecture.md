...
| D-31 | ตำแหน่งของตกแต่งอิง UV + reproject ทุกเฟรมที่ document เปลี่ยน | InstancedMesh ต่อ catalog entry, `projectUvToSurface` (A-11) หา world position ใหม่เสมอ | InstancedMesh ต่อเล็บ, ผูกของตกแต่งกับ world position ตรง ๆ |

---

### DECISION D-31 — ของตกแต่งอิง UV เหมือนเล็บ (D-10) — และบทเรียนสำคัญเรื่อง useMemo กับ StrictMode

**อะไร**: ของตกแต่งแต่ละชิ้นเก็บเป็น `(u, v, rotation, scale, catalogId)` (schema เดิมใน
`packages/contracts/src/design.ts` ตั้งแต่ก่อนรอบนี้) เรนเดอร์ด้วย `InstancedMesh` หนึ่งตัวต่อ
catalog entry ครอบคลุมทั้งมือ ตำแหน่งจริงในโลกคำนวณใหม่จาก `projectUvToSurface` (A-11)
ทุกครั้งที่ document เปลี่ยน (เพิ่ม/ลบ/ย้ายของตกแต่ง หรือเปลี่ยนทรง/ความยาวเล็บที่มีของตกแต่ง
ติดอยู่) เลือกของตกแต่งด้วยระยะทาง UV ที่ใกล้ที่สุดจากจุดคลิก (`nearestDecoration`) แทนการ
raycast กับ `InstancedMesh` ที่เรนเดอร์จริงแล้วแปลง `instanceId` กลับ — pointer-down บนเล็บ
ให้พิกัด UV มาอยู่แล้วผ่าน pipeline เดียวกับการวาด (`picking.ts`) จึงไม่ต้องสร้างระบบ
hit-test คู่ขนาน ย้ายด้วยเมาส์ (คลิกเลือก+ลากในท่าเดียว) หมุน/ย่อขยายผ่านแผงตัวเลขเท่านั้น
ไม่มี 3D gizmo — ตัดสินใจไว้ตั้งแต่สเปก (D-28/D-29/D-30 ใน
`docs/superpowers/specs/2026-08-15-nail-decoration-design.md`)

**ทำไม**: เหตุผลเดียวกับ D-10 — ถ้าเก็บเป็น world position ของตกแต่งจะลอยหลุดจากเล็บทันที
ที่ผู้ใช้เปลี่ยนทรง/ความยาวเล็บหรือสัดส่วนมือ (ฟีเจอร์ที่เพิ่งทำเสร็จใน D-27) พิสูจน์แล้วบน
เบราว์เซอร์จริง: เปลี่ยนทรงเล็บเป็น `stiletto` ขณะมีของตกแต่งติดอยู่ → ของตกแต่ง reproject
ตำแหน่งใหม่ทันที ไม่ลอยไม่จม

**ของตกแต่งยังเป็น placeholder geometry ล้วน** — ยังไม่มี asset 3D จริง (Slice 5)
โครงสร้าง `CatalogEntry` ใน `decorationCatalog.ts` ออกแบบให้สลับ geometry factory เป็น
GLTF loader ได้ทีหลังโดยไม่ต้องแก้โค้ดที่เหลือ

**บทเรียนสำคัญที่พบระหว่างตรวจงานด้วยเบราว์เซอร์จริง (Task 11) — ห้ามลืม**: การทดสอบผ่าน
`vitest`/task review ของทั้ง `DecorationInstances.tsx` และ `TransformController.tsx` ผ่าน
หมดทุกอย่าง แต่พอเปิดเบราว์เซอร์จริงกลับไม่เห็นของตกแต่งเลยแม้แต่พิกเซลเดียว สาเหตุคือ
`useMemo` factory ที่สร้าง `InstancedMesh` เขียนผลข้างเคียงลง `useRef` Map ข้างในตัวมันเอง —
`<StrictMode>` (`apps/web/src/main.tsx`) เรียก factory ของ `useMemo` สองรอบโดยตั้งใจใน dev
เพื่อจับโค้ดที่ไม่บริสุทธิ์แบบนี้ ผลคือ `meshRefs.current` ปลายทางชี้ไปยัง `InstancedMesh`
คนละก้อนกับที่ React เก็บไว้เรนเดอร์จริง (`meshes.includes(meshRefs.current.get('gem'))`
เป็น `false` เมื่อตรวจสด) ทุกครั้งที่แก้ instance matrix ผ่าน ref จึงแก้ mesh ที่ไม่เคยถูก
ใส่เข้าฉากเลย — ข้อมูลตำแหน่งถูกต้องสมบูรณ์ 100% แต่ไม่ปรากฏบนจอแม้แต่นิดเดียว ไม่มี error
ใน console ด้วย (ไม่ใช่ exception เป็นแค่ scene graph ที่ไม่ตรงกับที่ตั้งใจ)

**วิธีแก้และกฎที่ต้องรักษาไปตลอด**: `useMemo`/`useState` factory ต้องบริสุทธิ์เสมอ — ห้าม
เขียนผลข้างเคียงลง ref/ตัวแปรนอกขอบเขต ถ้าต้องมี lookup map จาก object ที่สร้างใน `useMemo`
ให้ derive มันเป็น `useMemo` แยกอีกตัวจากผลลัพธ์ตัวแรก (`useMemo(() => new Map(meshes.map(...)),
[meshes])`) ไม่ใช่ ref ที่ populate ระหว่าง render — `DecorationInstances.tsx` มีคอมเมนต์
อธิบายเรื่องนี้ไว้ในโค้ดแล้ว ห้ามลบทิ้งหรือกลับไปใช้ pattern เดิม

**ทำไมไม่ปิด StrictMode แทน**: StrictMode คือสิ่งที่จับบั๊กนี้ได้ (ถ้าปิดไปบั๊กจะกลาย
เป็น "บางทีก็หาย บางทีก็ไม่หาย" ขึ้นกับ timing ของ concurrent rendering ใน production ซึ่ง
debug ยากกว่านี้มาก) การแก้ที่ต้นเหตุ (ทำ factory ให้บริสุทธิ์) ถูกกว่าการหลบเลี่ยงเสมอ

**ค่า `defaultScale` ใน catalog ถูกวัดใหม่จากโมเดลจริง**: ค่าตั้งต้นตอนเขียนแผน (0.06-0.08)
เป็นการเดาที่ไม่มีข้อมูลอ้างอิง วัดจริงแล้วว่าตำแหน่งบนผิวเล็บอยู่ห่างจากจุดกำเนิดโลกประมาณ
0.15 หน่วย ปรับ `defaultScale` ลงเหลือ 0.004–0.006 ให้ได้สัดส่วนที่เหมาะกับเล็บจริง — เป็น
ค่าปรับแต่งเชิงศิลป์ ไม่ใช่ค่าคงที่ทางเรขาคณิตที่ต้องแม่นยำ ปรับเพิ่มได้อีกเมื่อ Slice 5
ใส่ asset จริงเข้ามาแทน
