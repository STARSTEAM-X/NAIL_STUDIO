...
| ~~A-9~~ | ~~Community/Template = แชร์ + ❤️ ไลก์ + ↗ แชร์ + 🔁 remix + คอมเมนต์ + รายงาน (ไม่มี follow/notification)~~ → **แก้บางส่วน 2026-08-14**: **notification กลับเข้าขอบเขตแล้ว** ดู A-13 — ที่ยังตัดออกคือ **follow/feed ส่วนตัว** เท่านั้น | ถ้าต้องการ follow/feed ส่วนตัว เพิ่ม ~1 สัปดาห์ |
...
| **A-12** | **แชทระหว่างผู้ใช้กับร้าน** — พบ draft schema (`conversations`, `messages`, `message_reads`) ที่ไม่เคยถูกนำเข้าแผนหลักจนถึงฉบับนี้ (ตกหล่นจากรอบ audit ครั้งก่อน ไม่ใช่การตัดขอบเขตโดยเจตนาแบบ A-9) → **เพิ่มกลับเข้าแผนที่ Slice 7** ขอบเขตจำกัดเฉพาะ**คู่สนทนา 1:1 ผูกกับนัดหมาย** (ไม่ใช่ DM ทั่วไประหว่างผู้ใช้กับผู้ใช้ ไม่มี group chat) ดู `database.md §2.2` และ `implementation-plan.md` Slice 7 | ถ้าต้องการแชททั่วไประหว่างผู้ใช้ (ไม่ผูกกับนัดหมาย) ต้องเพิ่ม `conversation_members` แบบ many-to-many และเพิ่มเวลาอีก ~1 สัปดาห์ |
| **A-13** | **Notification** — พบ draft schema (`notifications`, `notification_deliveries`, `user_notification_settings`) ใน `Olddesine/Notification.txt` ที่ตกหล่นเช่นเดียวกับ A-12 (`NotificationBell.jsx` ของ CEPP ที่เคยบันทึกไว้ว่า "แปลงเป็น .tsx ใช้ต่อได้" §2 บรรทัด 222 ก็ไม่เคยมีตาราง backend รองรับ) → **เพิ่มกลับเข้าแผน** ตัดเหลือ **ตารางเดียว `notifications`** ไม่ยก `notification_deliveries`/`user_notification_settings` มาทั้งชุด (ยังไม่มี email/push จริงในระบบ — เป็น in-app เท่านั้น) เหตุการณ์ที่แจ้ง: ไลก์/คอมเมนต์/remix เทมเพลตของตัวเอง (Slice 5), สถานะนัดหมายเปลี่ยน + ข้อความแชทใหม่ (Slice 7) ดู `database.md §3.10B` และ `implementation-plan.md` Slice 5 + Slice 7 | ถ้าต้องการ push/email จริง เพิ่ม `notification_deliveries` + ผู้ให้บริการภายนอก (~1 สัปดาห์); ถ้าต้องการปิด/เปิดรายประเภท เพิ่ม `user_notification_settings` (~2 วัน) |
