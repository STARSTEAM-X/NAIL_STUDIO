...
## M5 · วัด M0 ใหม่หลังแบ่งย่อยเล็บ (Task 3) — ในเว็บแอปจริง ไม่ใช่ spike
...
ของโมเดลก่อน Task 3 (ดูกฎข้อ 2 ของเอกสารนี้) ส่วน M5 คือค่าเดียวกันวัดใหม่ **หลัง** Task 3
...
- **draw calls = 6** ในการวัดนี้ตรงกับจำนวน mesh ในฉาก (มือ 1 + เล็บ 5 = 6) ตามที่ M0
  ...
  ทุกเฟรมหลัง cache เสร็จ) ซึ่งวิธีวัดของ M5 (hook เฉพาะ `gl.draw*` แล้ว reset ทุก `rAF`)
